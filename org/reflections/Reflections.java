...
public class Reflections
{
    ...
    protected void scan() {
        ...
        if (Reflections.log != null && Reflections.log.isDebugEnabled()) {
            Reflections.log.debug("going to scan these urls: {}", this.configuration.getUrls());
        }
            ...
            try {
                if (executorService != null) {
                    ...
                    futures.add(executorService.submit(() -> {
                        if (Reflections.log != null) {
                            Reflections.log.debug("[{}] scanning {}", Thread.currentThread().toString(), url2);
                        }
                        this.scan(url2);
                        return;
                    }));
                ...
                if (inputsFilter == null || inputsFilter.test(path) || inputsFilter.test(fqn)) {
                    ...
                    for (final Scanner scanner : this.configuration.getScanners()) {
                        try {
                            ...
                            Reflections.log.debug("could not scan file {} in url {} with scanner {}", file.getRelativePath(), url.toExternalForm(), scanner.getClass().getSimpleName(), e);
                        }
                    }
                }
            }
