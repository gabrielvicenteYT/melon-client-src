...
public class WebSocketImpl implements WebSocket
{
    ...
    private boolean decodeHandshake(final ByteBuffer socketBufferNew) {
        ...
        try {
            try {
                if (this.role == Role.SERVER) {
                    ...
                    if (handshakestate == HandshakeState.MATCHED) {
                        try {
                            ...
                            this.log.trace("Closing due to invalid data exception. Possible handshake rejection", e3);
                            this.flushAndClose(e3.getCloseCode(), e3.getMessage(), false);
                            return false;
                        }
                        catch (RuntimeException e4) {
                            this.log.error("Closing since client was never connected", e4);
                ...
                this.log.trace("Closing due to invalid handshake", e5);
                this.close(e5);
            }
        }
    ...
    public void sendPing() throws NullPointerException {
        final PingFrame pingFrame = this.wsl.onPreparePing(this);
        if (pingFrame == null) {
