...
public class ConnectionImpl implements JdbcConnection, Session.SessionEventListener, Serializable
{
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                ...
                try {
                    ...
                    this.session.execSQL(null, "commit", -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
                }
    ...
    private void connectOneTryOnly(final boolean isForReconnect) throws SQLException {
        ...
        try {
            ...
            final String oldDb = this.getDatabase();
            this.session.setQueryInterceptors(this.queryInterceptors);
            this.initializePropsFromServer();
            if (isForReconnect) {
                this.setAutoCommit(oldAutoCommit);
    ...
    private int getLoginTimeout() {
        final int loginTimeoutSecs = DriverManager.getLoginTimeout();
    ...
    public boolean isSameResource(final JdbcConnection otherConnection) {
        synchronized (this.getConnectionMutex()) {
            ...
            final String otherCurrentDb = ((ConnectionImpl)otherConnection).database;
        ...
        try {
            ...
            if (!this.propertySet.getBooleanProperty(PropertyKey.cacheCallableStmts).getValue()) {
                ...
                synchronized (this.parsedCallableStatementCache) {
                    final CompoundCacheKey key = new CompoundCacheKey(this.getDatabase(), sql);
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                if (this.cachePrepStmts.getValue() && pstmt.isPoolable()) {
                    synchronized (this.serverSideStatementCache) {
                        final Object oldServerPrepStmt = this.serverSideStatementCache.put(new CompoundCacheKey(pstmt.getCurrentDatabase(), ((PreparedQuery)pstmt.getQuery()).getOriginalSql()), (ServerPreparedStatement)pstmt);
    ...
    private void rollbackNoChecks() throws SQLException {
        try {
            synchronized (this.getConnectionMutex()) {
                ...
                this.session.execSQL(null, "rollback", -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
            }
        }
        catch (CJException ex) {
            throw SQLExceptionsMapping.translateException(ex, this.getExceptionInterceptor());
    ...
    public PreparedStatement serverPrepareStatement(final String sql, final int autoGenKeyIndex) throws SQLException {
        try {
            final String nativeSql = this.processEscapeCodesForPrepStmts.getValue() ? this.nativeSQL(sql) : sql;
            final ClientPreparedStatement pStmt = ServerPreparedStatement.getInstance(this.getMultiHostSafeProxy(), nativeSql, this.getDatabase(), 1003, 1007);
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                ...
                try {
                    ...
                    if (needsSetOnServer) {
                        this.session.execSQL(null, autoCommitFlag ? "SET autocommit=1" : "SET autocommit=0", -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
                    }
                }
                finally {
                    if (this.autoReconnectForPools.getValue()) {
    ...
    public void setDatabase(final String db) throws SQLException {
        try {
            synchronized (this.getConnectionMutex()) {
                ...
                this.session.execSQL(null, query.toString(), -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
                this.database = db;
            }
        }
        catch (CJException ex) {
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                if (this.readOnlyPropagatesToServer.getValue() && this.versionMeetsMinimum(5, 6, 5) && (!this.useLocalSessionState.getValue() || readOnlyFlag != this.readOnly)) {
                    this.session.execSQL(null, "set session transaction " + (readOnlyFlag ? "read only" : "read write"), -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
                }
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                ...
                if (shouldSendSet) {
                    ...
                    this.session.execSQL(null, sql, -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
            ...
            synchronized (this.getConnectionMutex()) {
                ...
                if (jdbcCompliantTruncation.getValue()) {
                    ...
                    if (currentSqlMode == null || currentSqlMode.length() == 0 || !strictTransTablesIsSet) {
                        ...
                        this.session.execSQL(null, commandBuf.toString(), -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
        ...
        try {
            synchronized (this.getConnectionMutex()) {
                if (this.session.getSessionMaxRows() != max) {
                    ...
                    this.session.execSQL(null, "SET SQL_SELECT_LIMIT=" + ((this.session.getSessionMaxRows() == -1) ? "DEFAULT" : Integer.valueOf(this.session.getSessionMaxRows())), -1, null, false, (ProtocolEntityFactory<Resultset, NativePacketPayload>)this.nullStatementResultSetFactory, null, false);
                }
            }
        }
        catch (CJException ex) {
