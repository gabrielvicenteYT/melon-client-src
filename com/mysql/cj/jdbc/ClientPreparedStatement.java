...
public class ClientPreparedStatement extends StatementImpl implements JdbcPreparedStatement
{
    ...
    protected static ClientPreparedStatement getInstance(final JdbcConnection conn, final String sql, final String db) throws SQLException {
        return new ClientPreparedStatement(conn, sql, db);
    }
    
    protected static ClientPreparedStatement getInstance(final JdbcConnection conn, final String sql, final String db, final ParseInfo cachedParseInfo) throws SQLException {
    ...
    protected ClientPreparedStatement(final JdbcConnection conn, final String db) throws SQLException {
        super(conn, db);
    ...
    public ClientPreparedStatement(final JdbcConnection conn, final String sql, final String db) throws SQLException {
        this(conn, sql, db, null);
    }
    
    public ClientPreparedStatement(final JdbcConnection conn, final String sql, final String db, final ParseInfo cachedParseInfo) throws SQLException {
        this(conn, db);
        try {
            ...
            synchronized (this.checkClosed().getConnectionMutex()) {
                try {
                    ...
                    try {
                        ...
                        rs = ((NativeSession)locallyScopedConnection.getSession()).execSQL(this, null, maxRowsToRetrieve, (NativePacketPayload)sendPacket, createStreamingResultSet, this.getResultSetFactory(), metadata, isBatch);
                        if (timeoutTask != null) {
                            this.stopQueryTimer(timeoutTask, true, true);
                            timeoutTask = null;
                        }
    ...
    protected ClientPreparedStatement prepareBatchedInsertSQL(final JdbcConnection localConn, final int numBatches) throws SQLException {
        try {
            synchronized (this.checkClosed().getConnectionMutex()) {
                final ClientPreparedStatement pstmt = new ClientPreparedStatement(localConn, "Rewritten batch of: " + ((PreparedQuery)this.query).getOriginalSql(), this.getCurrentDatabase(), ((PreparedQuery)this.query).getParseInfo().getParseInfoForBatch(numBatches));
    ...
    @Override
    public byte[] getOrigBytes(final int parameterIndex) throws SQLException {
        synchronized (this.checkClosed().getConnectionMutex()) {
    ...
    public final int getCoreParameterIndex(final int paramIndex) throws SQLException {
