...
public class UpdatableResultSet extends ResultSetImpl
{
    ...
    public boolean absolute(final int row) throws SQLException {
        try {
            final boolean ret = super.absolute(row);
            if (this.onInsertRow) {
                this.onInsertRow = false;
            }
            ...
            for (int i = 0; i < fields.length; ++i) {
                ...
                if (fields[i].getOriginalTableName() != null) {
                    ...
                    if (tableOnlyName2 != null) {
                        ...
                        updColumnNameToIndex = this.getColumnsToIndexMapForTableAndDB(this.db, tableOnlyName2);
                    }
                }
    ...
    public void insertRow() throws SQLException {
        try {
            synchronized (this.checkClosed().getConnectionMutex()) {
                ...
                for (int i = 0; i < fields.length; ++i) {
                    newRow[i] = (byte[])(this.inserter.isNull(i + 1) ? null : this.inserter.getBytesRepresentation(i + 1));
    ...
    public boolean previous() throws SQLException {
        try {
            final boolean ret = super.previous();
            if (this.onInsertRow) {
                this.onInsertRow = false;
            }
            if (this.doingUpdates) {
        ...
        for (int numKeys = this.primaryKeyIndicies.size(), i = 0; i < numKeys; ++i) {
            ...
            if (!this.doingUpdates && !this.onInsertRow) {
                ...
                if (updateInsertStmt.isNull(index + 1) || dataFrom.length == 0) {
                    ...
                    final byte[] origBytes = updateInsertStmt.getOrigBytes(i + 1);
                    if (origBytes != null) {
    ...
    public boolean relative(final int rows) throws SQLException {
        try {
            final boolean ret = super.relative(rows);
            if (this.onInsertRow) {
                this.onInsertRow = false;
            }
            if (this.doingUpdates) {
