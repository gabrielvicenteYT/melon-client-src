...
public class ExprParser
{
    ...
    MysqlxExpr.Expr buildUnaryOp(final String name, final MysqlxExpr.Expr param) {
        final String opName = "-".equals(name) ? "sign_minus" : ("+".equals(name) ? "sign_plus" : name);
        final MysqlxExpr.Operator op = MysqlxExpr.Operator.newBuilder().setName(opName).addParam(param).build();
        return MysqlxExpr.Expr.newBuilder().setType(MysqlxExpr.Expr.Type.OPERATOR).setOperator(op).build();
    }
    ...
    MysqlxExpr.Expr addSubIntervalExpr() {
        ...
        while ((this.currentTokenTypeEquals(TokenType.PLUS) || this.currentTokenTypeEquals(TokenType.MINUS)) && this.nextTokenTypeEquals(TokenType.INTERVAL)) {
            ...
            ++this.tokenPos;
            final MysqlxExpr.Operator.Builder builder = MysqlxExpr.Operator.newBuilder().addParam(lhs);
    ...
    MysqlxExpr.Expr ilriExpr() {
        ...
        final List<TokenType> expected = Arrays.asList(TokenType.IS, TokenType.IN, TokenType.LIKE, TokenType.BETWEEN, TokenType.REGEXP, TokenType.NOT, TokenType.OVERLAPS);
    ...
    static {
        ...
        ExprParser.reservedWords.put("overlaps", TokenType.OVERLAPS);
    }
